...
function portfolio_messages_page()
{
    global $wpdb;
    ...
    if (isset($_POST['delete_message_id']) && is_numeric($_POST['delete_message_id'])) {
        ...
        // Allow deleting any message (admin view)
        $wpdb->delete($table, [
            'id' => $delete_id
        ]);
            ...
            <tbody>
                <?php if ($messages) : ?>
                    <?php foreach ($messages as $msg): ?>
                        <?php $recipient = get_userdata($msg->receiver_id); ?>
                        <tr>
                            <td><?php echo ucfirst(esc_html($msg->sender_name ?: 'Anonymous')); ?></td>
                            <td><?php echo esc_html($msg->sender_email); ?></td>
                            <td><?php echo ucfirst(esc_html($recipient ? $recipient->display_name : 'Unknown')); ?></td>
